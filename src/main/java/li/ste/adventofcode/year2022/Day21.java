        ...
        while (scanner.hasNextLine()) {
            ...
            if (matcher.find()) {
                ...
                if (matcher.find()) {
                    ...
                    throw new AdventOfCodeException("NO MATCH!");
                }
            }
        }
        setSolution1((long)monkeys.get("root").getResult(monkeys));
    ...
    private static class OperatorMonkey implements Monkey {
        ...
        public double getResult(Map<String, Monkey> monkeys) {
            return switch (operation) {
                ...
                case '+' -> monkeys.get(monkey1).getResult(monkeys) + monkeys.get(monkey2).getResult(monkeys);
                case '-' -> monkeys.get(monkey1).getResult(monkeys) - monkeys.get(monkey2).getResult(monkeys);
                default -> throw new AdventOfCodeException("unknown operation: " + operation);
            };
        }
    }
}
