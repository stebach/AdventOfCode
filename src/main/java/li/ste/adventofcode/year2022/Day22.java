...
public class Day22 extends Day {
    ...
    public void run() {
        ...
        if (grid.size() / 3 == grid.get(0).length / 4) {
            ...
            throw new AdventOfCodeException("size ratio not recognized");
        }

        setSolution1(solve(false));
        setSolution2(solve(true));
        //144152
        //150227 -> too high
    }
                ...
                for (int i = 0; i < instruction; i++) {
                    ...
                    switch (grid.get(newY)[newX]) {
                        ...
                        case '#':
                            break movement;
                        default:
                            throw new AdventOfCodeException("unknown map tile \"" + grid.get(newY)[newX] + "\" " + newX + " " + newY + " | " + direction[0] + " / " + direction[1]);
                    }
                }
            }
            isTurn = !isTurn;
    ...
    private int[] fixData(int x, int y, int[] direction, boolean part2) {
        int[] newDirection = direction.clone();
        if (!part2) {
            if (grid.size() <= y) {
                y = 0;
